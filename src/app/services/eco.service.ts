...
import { ORDEN_DIFICULTAD, DIFICULTAD_POR_NIVEL } from '../shared/constantes';
...
export class EcoService {
  ...
  recomendaciones = computed(() => {
    ...
    if (!user || user.isAdmin) {
      return rutas.sort((a, b) => b.popularidad - a.popularidad).slice(0, 3);
    }

    const ideal = ORDEN_DIFICULTAD[DIFICULTAD_POR_NIVEL[user.nivel]];
    const tiposFavoritos = rutas.filter(r => user.favoritos.includes(r.id)).map(r => r.tipo);
    const distancia = (r: Ruta) => Math.abs(ORDEN_DIFICULTAD[r.dificultad] - ideal);
    const puntaje = (r: Ruta) =>
      r.popularidad - distancia(r) * 40 + (tiposFavoritos.includes(r.tipo) ? 20 : 0);

    return rutas
      .filter(r => distancia(r) < 2)
      .sort((a, b) => puntaje(b) - puntaje(a))
      .slice(0, 3);
  });

  getFilteredRutas(filters: Filtros) {
